...
namespace HREngine.Bots
{
                                    ...
                                    //Deal 2 damage to a minion. If it dies,deal 3 damage to the enemy hero.
    {
        public override void onCardPlay(Playfield p, bool ownplay, Minion target, int choice)
        {
            ...
            // 如果随从在受到伤害后死亡（生命值小于等于0），对敌方英雄造成3点伤害
            if (targetHpBefore > 0 && target.Hp <= 0)
            {
                // 计算对英雄的伤害（考虑法术伤害加成）
                int heroDmg = (ownplay) ? p.getSpellDamageDamage(3) : p.getEnemySpellDamageDamage(3);
                p.minionGetDamageOrHeal(p.enemyHero, heroDmg);
            }
        }
        ...
        public override PlayReq[] GetPlayReqs()
        {
            return new PlayReq[] {
                ...
                // 注释掉敌方目标限制，因为有些卡牌可以对自己随从使用（如牺牲契约等）
                // 但精神灼烧通常只能对敌方随从使用，根据实际卡牌效果决定是否取消注释
