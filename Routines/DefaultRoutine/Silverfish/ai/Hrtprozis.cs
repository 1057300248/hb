namespace HREngine.Bots
{
    ...
    public class Hrtprozis  //Hrtprozis是兄弟中非常重要的一个类，记录着从兄弟内部数据中获取到的各种信息
    {
        ...
        public List<CardDB.cardIDEnum> ownAuraList = new List<CardDB.cardIDEnum>();//我方光环列表
        ...
        //使用过的上一张非潜行者的职业牌
        public Handmanager.Handcard lastNonRogueClassCardPlayed = null;
        //冲浪章鱼
        ...
        //playfield 帕奇斯
        public bool playfieldPatchesInDeck = true;
        ...
        CardDB cdb;//卡牌数据
    public int anzOwnLifesteal = 0;//我方吸血计数器
    public int anzEnemyLifesteal = 0;//敌方吸血计数器
        ...
        public void updateInfestor(int num)
        {//感染者
            我方感染者 = num;
        }
        ...
        public void updateauraStuff(List<string> ownaura, int numEnemSec)
        {//更新光环状态
            this.ownAuraList.Clear();
            foreach (string s in ownaura)
            {
                this.ownAuraList.Add(cdb.cardIdstringToEnum(s));
            }
            this.enemySecretCount = numEnemSec;
        }
        ...
        public void updateCorpses(int 本局获得尸体数, int 本局消耗尸体数)
        {
            this.本局获得尸体数 = 本局获得尸体数;
            this.本局消耗尸体数 = 本局消耗尸体数;
        }
        ...
        private Minion createNewMinion(Handmanager.Handcard hc, int id)
        {
            ...
            if (hc.card.superwindfury) m.superwindfury = true;
        ...
        {
            help.logg("player:");
            help.logg(this.numMinionsPlayedThisTurn + " " + this.cardsPlayedThisTurn + " " + this.ueberladung + " " + this.lockedMana + " " + this.本局获得尸体数 + " " + this.本局消耗尸体数 + " " + this.ownPlayerController);

            help.logg("ownhero:");
            help.logg((this.heroname == HeroEnum.None ? this.heronameingame : this.heroname.ToString()) + " " + this.ownHero.Hp + " " + this.ownHero.maxHp + " " + this.ownHero.armor + " " + this.ownHero.immuneWhileAttacking + " " + this.ownHero.immune + " " + this.ownHero.entitiyID + " " + this.ownHero.Ready + " " + this.ownHero.numAttacksThisTurn + " " + this.ownHero.frozen + " " + this.ownHero.Angr + " " + this.ownHero.tempAttack + " " + this.enemyHero.stealth + " " + this.ownHero.divineshild
                + ( this.ownHero.enchs.Count > 0 ? " 附魔:" + this.ownHero.enchs : "" )
                );
            ...
            string aura = "";
            foreach (CardDB.cardIDEnum aur in this.ownAuraList)
            {
                aura += aur + " ";
            }
            help.logg("auras: " + aura);
        ...
        public void printOwnMinions()
        {
            ...
            foreach (Minion m in this.ownMinions)
            {
                ...
                if (m.rush>0) mini += " rush(1)";
                if (m.superwindfury) mini += " swndfr";
                ...
                if (m.enchs.Count > 0) mini += " 附魔:" + m.enchs;
