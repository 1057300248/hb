...
namespace HREngine.Bots
{
    ...
    public class Minion
    {
        ...
        public string Status
        {
            get
            {
                ...
                return "{" + zonepos.ToString() + "} " + " (" + Angr + "/" + Hp + ") " + handcard.card.nameCN + "\n " +
                    (frozen ? "[冻结]" : "") + (!Ready || cantAttack ? "[无法攻击]" : "[可攻击]") + (extraAttacksThisTurn > 0 ? "[额外攻击次数:" + extraAttacksThisTurn + "]" : "") + (windfury ? "[风怒]" : "") + (superwindfury ? "[超级风怒]" : "") + (taunt ? "[嘲讽]" : "")
                    + (divineshild ? "[圣盾]" : "") + (stealth ? "[隐身]" : "") + (immune ? "[免疫]" : "") + (untouchable ? "[不可触碰]" : "") + (lifesteal ? "[吸血]" : "")
                     + (dormant != 0 ? "[休眠(" + dormant.ToString() + ")]" : "") + (reborn ? "[复生]" : "") + (zerg ? "[异虫]" : "") + (poisonous ? "[剧毒]" : "")
                      + (Spellburst ? "[法术迸发]" : "") + (HonorableKill ? "[荣誉击杀]" : "") + (Overkill ? "[超杀]" : "") + (Elusive ? "[扰魔]" : "" + (Aura ? "[光环]" : ""));
            }
        }

        public bool TitanAbilityUsable = false;
        public bool titanAbilityUsedThisTurn = false; // 标记泰坦本回合是否已使用过技能
        ...
        public Minion(Minion m)
        {
            ...
            this.enchs = m.enchs;
            this.terran = m.terran;
            this.zerg = m.zerg;
            this.terran = m.terran;
            this.protoss = m.protoss;
        ...
        public void getDamageOrHeal(int dmg, Playfield p, bool isMinionAttack, bool dontCalcLostDmg)
        {
            ...
            if (!silenced)
            {
                handcard.card.sim_card.ReturnMinionReceiveDamage(this, p, damage);
                Hp -= damage;
            }
            else
            {
        ...
        public void minionDied(Playfield p)
        {
            ...
            if (own)
            {
                ...
                /*                 if (handcard.card.race == CardDB.Race.BEAST)
                                {
                                    p.tempTrigger.ownBeastDied++;
                                }
        ...
        public void updateReadyness()
        {
            ...
            switch (handcard.card.type)
            {
                case CardDB.cardtype.MOB:
                    {
                        ...
                        else
                        {
                            ...
                            {
                                if (numAttacksThisTurn < 1 + (!superwindfury && windfury ? 1 : 0) + (superwindfury ? 3 : 0) + extraAttacksThisTurn)
                                {
                                    Ready = true;
                                }
                            }
                ...
                case CardDB.cardtype.HERO:
                    ...
                    break;
                case CardDB.cardtype.LOCATION://地标
                    {
                        // 检查地标是否处于冷却状态
                        if (CooldownTurn == 0)
                            Ready = true;
        ...
        public override bool Equals(object obj)
        {
            ...
            return anzGotDmg == minion.anzGotDmg &&
                   ...
                   superwindfury == minion.superwindfury &&
        ...
        public override int GetHashCode()
        {
            ...
            hashCode = hashCode * -1521134295 + superwindfury.GetHashCode();
