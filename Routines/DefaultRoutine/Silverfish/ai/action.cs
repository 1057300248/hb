        ...
        public string GetActionString()
        {
            ...
            switch (this.actionType)
            {
                case actionEnum.endturn:
                    ...
                    break;
                case actionEnum.playcard:
                    // 安全访问 card 和 target 字段，防止空引用
                    string cardInfo = "无";
                    if (this.card != null && this.card.card != null)
                    {
                        cardInfo = this.card.card.chnInfo();
                    }
                    string targetInfo = "空";
                    if (this.target != null)
                    {
                        targetInfo = this.target.info();
                    }
                    str.AppendFormat("打出 {0} 目标 {1} 惩罚值：{2} ", cardInfo, targetInfo, this.penalty);
                    break;
                case actionEnum.attackWithHero:
                    // 安全访问 target 字段
                    string heroTargetInfo = "空";
                    if (this.target != null)
                    {
                        heroTargetInfo = this.target.info();
                    }
                    str.AppendFormat("让英雄攻击 {0} 惩罚值：{1} ", heroTargetInfo, this.penalty);
                    break;
                case actionEnum.useHeroPower:
                    // 安全访问 target 字段
                    string hpTargetInfo = "空";
                    if (this.target != null)
                    {
                        hpTargetInfo = this.target.info();
                    }
                    str.AppendFormat("使用英雄技能 目标 {0} 惩罚值：{1} ", hpTargetInfo, this.penalty);
                    break;
                case actionEnum.attackWithMinion:
                    // 安全访问 own 和 target 字段
                    string ownMinionInfo = "未知随从";
                    if (this.own != null)
                    {
                        ownMinionInfo = this.own.info();
                    }
                    string attackTargetInfo = "空";
                    if (this.target != null)
                    {
                        attackTargetInfo = this.target.info();
                    }
                    str.AppendFormat("使用随从 {0} 攻击 {1} 惩罚值：{2} ", ownMinionInfo, attackTargetInfo, this.penalty);
                    break;
                case actionEnum.trade:
                    // 安全访问 card 字段
                    string tradeCardInfo = "无";
                    if (this.card != null && this.card.card != null)
                    {
                        tradeCardInfo = this.card.card.chnInfo();
                    }
                    str.AppendFormat("使用随从 {0} 交易 惩罚值：{1} ", tradeCardInfo, this.penalty);
                    break;
                case actionEnum.useLocation:
                    // 安全访问 own 和 target 字段
                    string locationInfo = "未知地标";
                    if (this.own != null)
                    {
                        locationInfo = this.own.info();
                    }
                    string locTargetInfo = "空";
                    if (this.target != null)
                    {
                        locTargetInfo = this.target.info();
                    }
                    str.AppendFormat("使用地标 {0} 目标 {1} 惩罚值：{2} ", locationInfo, locTargetInfo, this.penalty);
                    break;
                case actionEnum.useTitanAbility:
                    string suffix = "";
                    // 安全访问 own 字段
                    ...
                    {
                        str.Append("使用泰坦技能 [无效目标] 惩罚值：" + this.penalty);
                        break;
                    }
                    switch (this.own.handcard.card.cardIDenum.ToString())
                    {
                    ...
                    CardDB.Card card = CardDB.Instance.getCardDataFromID(CardDB.Instance.cardIdstringToEnum(this.own.handcard.card.cardIDenum.ToString() + suffix));
                    string titanSkillName = "未知技能";
                    if (card != null)
                    {
                        titanSkillName = card.nameCN.ToString();
                    }
                    string titanTargetInfo = "无";
                    if (this.target != null && this.target.handcard != null)
                    {
                        titanTargetInfo = this.target.handcard.card.nameCN.ToString();
                    }
                    str.AppendFormat("使用泰坦技能 {0} 目标{1} 惩罚值：{2} ", titanSkillName, titanTargetInfo, this.penalty);
                    break;
                case actionEnum.forge:
                    // 安全访问 own 字段
                    string forgeMinionInfo = "未知随从";
                    if (this.own != null)
                    {
                        forgeMinionInfo = this.own.info();
                    }
                    str.AppendFormat("使用随从 {0} 锻造 惩罚值： {1}", forgeMinionInfo, this.penalty);
                    break;
            }
            return str.ToString();
        }
