...
namespace HREngine.Bots
{
    public class SimTemplate
    {
        ...
        /// <param name="target">选定目标</param>
        /// <param name="choice">选项（抉择、发现等）</param>
        public virtual void 打出随从带抉择(Playfield p, Minion own, Minion target, int choice)
        {
            return;
        }
        ...
        /// <param name="p">场面</param>
        /// <param name="own">是否自己打出</param>
        public virtual void onOutcast(Playfield p, Minion target, bool own)
        {
            return;
        }
        /// <summary>
        ...
        public virtual void afterMinionAttack(Playfield p, Minion attacker, Minion target, int damage)
        {
            // 默认实现为空。子类可以根据需求重写此方法。
            return;
        }
        ...
        /// <param name="p">游戏场地对象，包含游戏状态信息。</param>
        /// <param name="triggerEffectMinion">触发此效果的随从。</param>
        /// <param name="attackingMinion">攻击的的随从。</param>
        public virtual void onMinionAttacks(Playfield p, Minion triggerEffectMinion, Minion attackingMinion)
        {
            // 默认空实现
            return;
        }
        ...
        /// </summary>
        /// <param name="p">游戏场地对象，包含当前游戏状态的信息。</param>
        /// <param name="triggerMinion">星舰所属的随从对象。</param>
        /// <param name="target">指向的的目标随从。</param>
        public virtual void useStarship(Playfield p, Minion triggerMinion, Minion target)
        {
            return;
        }

        // 在 SimTemplate 类中添加
        public virtual void ReturnMinionReceiveDamage(Minion m, Playfield p, int dmg)
        {
            return;
            // 默认实现为空或根据需要添加适当逻辑
        }

        public virtual int GetManaCost(Playfield p, int manaCost)
        {
            return manaCost; // 默认返回原法力值消耗
        }
