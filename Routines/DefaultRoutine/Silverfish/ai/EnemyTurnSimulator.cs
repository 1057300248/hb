    ...
    public class EnemyTurnSimulator
    {
        ...
        public void simulateEnemysTurn(Playfield rootfield, bool simulateTwoTurns, bool playaround, bool print, int pprob, int pprob2)
        {
            if (rootfield.bestEnemyPlay == null)
            {
                ...
                // 当启用“playaround”策略且当前对局中不存在洛欧塞布（Loatheb）时，尝试模拟对手使用法术反制（Counterspell）的情况，
                // 并评估该假设对当前局面价值的影响。若模拟后局面价值提升，则重置候选动作列表，仅保留一个基于当前场景区拷贝的新起始回合状态，
                // 用于后续更精确的决策模拟。
                if (playaround && !rootfield.法术费用增加)
                {
                ...
                //play ability!
                if (rootfield.enemyHeroPowerCostLessOnce <= 0 && posmoves[0].enemyAbilityReady && enemMana >= rootfield.enemyHeroAblility.manacost && posmoves[0].enemyHeroAblility.card.canplayCard(posmoves[0], 0, false) && !rootfield.法术费用增加)
                {
