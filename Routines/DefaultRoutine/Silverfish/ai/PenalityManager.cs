namespace HREngine.Bots
{
    ...
    public class PenalityManager
    {
        ...
        public int getSilencePenality(CardDB.cardNameEN name, Minion target, Playfield p)
        {
            ...
            else
            {
                ...
                if (this.silenceDatabase.ContainsKey(name))
                {
                    ...
                    if (target.Angr <= target.handcard.card.Attack && target.maxHp <= target.handcard.card.Health && !target.taunt && !target.windfury && !target.divineshild && !target.poisonous && !target.reborn && !target.lifesteal && !this.specialMinions.ContainsKey(name))
                    {
        ...
        /// <summary>
        /// 计算带有随从惩罚的攻击值，用于AI决策攻击目标
        /// </summary>
        /// <param name="m">攻击的随从对象，可以为null</param>
        /// <param name="p">当前游戏场面对象</param>
        /// <param name="target">攻击的目标随从或英雄</param>
        /// <returns>计算后的攻击值，如果目标不可攻击则返回1000</returns>
        public int getAttackWithMininonPenality(Minion m, Playfield p, Minion target)
        {
            // 检查目标是否有效（不存在、不可触及、是毁灭战舰或生命值小于等于0）
        ...
        /// <summary>
        /// 获取带有英雄惩罚的攻击值，用于AI决策计算
        /// </summary>
        /// <param name="target">目标随从对象</param>
        /// <param name="p">游戏战场状态对象</param>
        /// <returns>计算后的攻击值，如果目标无效则返回1000</returns>
        public int getAttackWithHeroPenality(Minion target, Playfield p)
        {
            // 检查目标是否有效：空目标、不可触及、毁灭战舰或血量小于等于0的情况
            if (target == null || target.untouchable || target.handcard.card.nameCN == CardDB.cardNameCN.毁灭战舰 || target.Hp <= 0) return 1000;
        ...
        private void setupDamageDatabase()
        {
            ...
            this.maycauseharmDatabase.Add(CardDB.cardNameEN.morbidswarm, 1);
        ...
        private void setupsilenceDatabase()
        {
            ...
            this.silenceDatabase.Add(CardDB.cardNameEN.deafen, 1);//致聋术
        ...
        private void setupSilenceTargets()
        {
            ...
            // 泰坦
            silenceTargets.Add(CardDB.cardNameEN.khazgoroth, 0);
            silenceTargets.Add(CardDB.cardNameEN.aggramartheavenger, 0);
            silenceTargets.Add(CardDB.cardNameEN.norgannon, 0);
            silenceTargets.Add(CardDB.cardNameEN.golganneththethunderer, 0);
            silenceTargets.Add(CardDB.cardNameEN.amitusthepeacekeeper, 0);
            silenceTargets.Add(CardDB.cardNameEN.argustheemeraldstar, 0);
            silenceTargets.Add(CardDB.cardNameEN.amanthul, 0);
            silenceTargets.Add(CardDB.cardNameEN.theprimus, 0);
            silenceTargets.Add(CardDB.cardNameEN.sargerasthedestroyer, 0);
            silenceTargets.Add(CardDB.cardNameEN.eonarthelifebinder, 0);
            silenceTargets.Add(CardDB.cardNameEN.yoggsaronunleashed, 0);

            silenceTargets.Add(CardDB.cardNameEN.infestor, 0);// 感染者
            silenceTargets.Add(CardDB.cardNameEN.carrier, 0);//航母 
            silenceTargets.Add(CardDB.cardNameEN.thunderbringer, 0);//奔雷骏马
        }
        ...
        private void setupChooseDatabase()
        {
            ...
            this.choose1database.Add(CardDB.cardNameEN.morbidswarm, CardDB.cardIDEnum.EDR_813a);

            this.choose2database.Add(CardDB.cardNameEN.morbidswarm, CardDB.cardIDEnum.EDR_813b);
        ...
        /// 获取使用星舰的惩罚值
        /// </summary>
        /// <param name="StarshipMinion">星舰对应的随从对象</param>
        /// <param name="target">星舰的目标随从</param>
        /// <param name="p">当前场面</param>
        /// <returns>惩罚值</returns>
        public int getUseStarshipPenality(Minion StarshipMinion, Minion target, Playfield p)
        {
            int enfaceReward = 0;
            if (target != null)
            {
            ...
            // 调用AI的基础逻辑来获取惩罚值，并减去奖励值
            return ai.botBase.getUseStarshipPenality(StarshipMinion, target, p) - enfaceReward;
        }
    }
