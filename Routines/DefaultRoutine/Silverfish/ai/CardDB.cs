...
namespace HREngine.Bots
{
    public partial class CardDB
    {
        ...
        public class Card
        {
            ...
            public bool superwindfury = false;//超级风怒
            ...
            public List<Minion> getTargetsForCard(Playfield p, bool isLethalCheck, bool own)
            {
                ...
                if (extraParam)
                {
                    ...
                    if (REQ_STEADY_SHOT)
                    {
                        if ((p.我们有热砂港狙击手 && own) || (p.敌方有热砂港狙击手 && !own))
                        {
                            foreach (Minion m in targets)
                            {
            ...
            public List<Minion> getTargetsForHeroPower(Playfield p, bool own)
            {
                ...
                targetsForHeroPower.RemoveAll(minion => minion != null &&
                                                        minion.handcard != null &&
                                                        minion.handcard.card != null &&
                                                        minion.handcard.card.type == CardDB.cardtype.STARSHIP);
            ...
            public List<Minion> getTargetsForStarship(Playfield p, bool isLethalCheck, bool own)
            {
                List<Minion> retval = new List<Minion>();
                ...
                if (targetAll)
                {
                    ...
                    if (targetAllFriendly != targetAllEnemy)
                    {
                        if (targetAllFriendly)
                        {
                            ...
                            {
                                if (!m.untouchable && !isLocationOnCooldown(m)) // 检查地标是否冷却
                                    targets.Add(m);
                            }
                        }
                        else
            ...
            /// 判断地标是否处于冷却状态
            /// </summary>
            /// <param name="location">地标随从</param>
            /// <returns>true 表示冷却中，false 表示可用</returns>
            private bool isLocationOnCooldown(Minion location)
            {
                // 假设地标冷却状态存储在 Minion 的某个字段中（例如 CooldownTurn）
                return location.CooldownTurn > 0;
            }
            ...
            public int calculateManaCost(Playfield p)//calculates the mana from orginal mana, needed for back-to hand effects and new draw
            {
                ...
                switch (this.type)
                {
                    case cardtype.MOB://随从
                        if (p.anzOwnAviana > 0) retval = 1;//av娜

                        if (p.纳迦海巫 > 0) retval = 5;//纳迦海巫
                    ...
                    case cardtype.SPELL://法术
                        ...
                        if (p.纳迦海巫 > 0) retval = 5;//纳迦海巫
                        break;
                    case cardtype.WEAPON://武器
                        offset -= p.blackwaterpirate * 2;//黑水海盗
                        if (this.deathrattle) offset += p.ownDRcardsCostMore;
                        if (p.纳迦海巫 > 0) retval = 5;//纳迦海巫
                        break;
                    case cardtype.LOCATION://地标
                        if (p.纳迦海巫 > 0) retval = 5;//纳迦海巫
                        break;
                }
            ...
            public int getManaCost(Playfield p, int currentcost)
            {
                ...
                switch (this.type)
                {
                    ...
                    case cardtype.MOB:
                        ...
                        if (this.battlecry && p.尼鲁巴蛛网领主 != p.startedWithnerubarweblord)
                        {
                            offset += (p.尼鲁巴蛛网领主 - p.startedWithnerubarweblord) * 2;
                        }
                        ...
                        if (p.龙族减费 != p.龙族减费牌刚使用 && (TAG_RACE)this.race == TAG_RACE.DRAGON)
                        {
                            offset += (p.龙族减费牌刚使用 - p.龙族减费) * 2;
                        }
                    ...
                    case cardtype.SPELL:
                        ...
                        // 判断当前法术是否属于敌方（通过检查敌方手牌）
                        // 实际作用不大 这种对面给我们造成整个回合的费用计算hb会计算
                        bool isEnemySpell = p.enemyHand.Exists(hc => hc.card.cardIDenum == this.cardIDenum);

                        // 如果洛欧塞布效果激活且为敌方法术，则增加费用
                        if (p.法术费用增加 && isEnemySpell)
                        {
                            offset += p.洛欧塞布法力值增加量;
                        }
                ...
                switch (this.nameEN)
                {
                    ...
                    case CardDB.cardNameEN.fleshgiant:
                        ...
                        break;
                    // 口渴的流浪者
                    case CardDB.cardNameEN.thirstydrifter:
                        retval = retval + offset - p.一费卡牌使用次数;
                        break;                        
                    case CardDB.cardNameEN.crush:
                        ...
                        break;
					case CardDB.cardNameEN.boulderingbuddy:
                        if (p.ownMaxMana > 9)
                            retval = 1 + offset;
                        break;
            ...
            foreach (XmlElement entity in entities)
            {
                ...
                foreach (XmlElement tag in entity.ChildNodes)
                {
                    ...
                    if ("ReferencedTag".Equals(tag.Name))
                    {
                        ...
                        if (tag.GetAttribute("enumID") == "1207")
                        {
                            card.superwindfury = true;
                        }
                        if (tag.GetAttribute("enumID") == "190")
                        {
                            card.tank = true;
                        }
                        if (tag.GetAttribute("enumID") == "3457")
                        {
                            card.zerg = true;
                        }
                        if (tag.GetAttribute("enumID") == "189")
                        {
                            card.windfury = true;
                        }
                        continue;
                ...
                if (card.nameCN == cardNameCN.步移山丘)
                {
                    card.superwindfury = true;
                }
                if (card.nameCN == cardNameCN.猎潮者耐普图隆)
                {
                    card.windfury = true;
                }
                // card.updateDIYCard();
        ...
        // 定义映射表
        public static Dictionary<string, Race> raceMapping = new Dictionary<string, Race>(StringComparer.OrdinalIgnoreCase)
        {
        ...
        /// <summary>
        /// 将List<string>转换为List<Race>
        /// </summary>
        /// <param name="raceStrings"></param>
        /// <returns></returns>
        public static List<Race> ConvertStringListToRaceList(List<string> raceStrings)
        {
            List<Race> raceList = new List<Race>();

            foreach (string raceStr in raceStrings)
            {
                // 清理输入字符串
                string cleanedRaceStr = raceStr.Trim();

                Race race;

                // 使用映射表进行转换
                if (raceMapping.TryGetValue(cleanedRaceStr, out race))
                {
                    raceList.Add(race);
            }
            }
