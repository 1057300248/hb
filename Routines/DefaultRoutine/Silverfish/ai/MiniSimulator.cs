            ...
            foreach (targett t in oldlist)
            {
                ...
                if ((own && t.target >= 10 && t.target <= 19) || (!own && t.target >= 0 && t.target <= 9))
                {
                    ...
                    foreach (Minion mnn in temp)
                    {
                        ...
                        {
                            if (mnn.Angr == m.Angr && mnn.Hp == m.Hp && mnn.divineshild == m.divineshild && mnn.taunt == m.taunt && mnn.zerg == m.zerg && mnn.poisonous == m.poisonous && mnn.lifesteal == m.lifesteal) goingtoadd = false;
                            continue;
                        }
                        ...
                        if (isSpecial == otherisSpecial && !m.silenced && !mnn.silenced) // same are special
                        {
                            ...
                            if (mnn.Angr == m.Angr && mnn.Hp == m.Hp && mnn.divineshild == m.divineshild && mnn.reborn == m.reborn && mnn.taunt == m.taunt && mnn.zerg == m.zerg && mnn.poisonous == m.poisonous && mnn.lifesteal == m.lifesteal && mnn.Spellburst == m.Spellburst) goingtoadd = false;
                            continue;
                        }
