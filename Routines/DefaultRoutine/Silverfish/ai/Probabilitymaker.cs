...
/// <summary>
/// 表示墓地中的一个物品，包含该物品的所有者、实体ID、卡牌ID以及状态信息。
/// </summary>
public struct GraveYardItem
{
    ...
    /// 初始化一个新的墓地物品实例。
    /// </summary>
    /// <param name="id">物品对应的卡牌ID。</param>
    /// <param name="entityId">物品的唯一实体ID。</param>
    /// <param name="own">指示该物品是否属于当前玩家。</param>
    /// <param name="status">物品在墓地中的状态。</param>
...
public class SecretItem   // 防奥秘类，通过修改下面的true -> false，可以去掉一些要防的奥秘，防冰箱和aoe足够
    ...
    /// <summary>
    /// 存储己方墓地中的卡牌及其数量。
    /// 键为卡牌ID（CardDB.cardIDEnum），值为该卡牌在墓地中的数量。
    /// </summary>
    ...
    /// <summary>
    /// 存储己方已丢弃的卡牌及其数量。
    /// 键为卡牌ID（CardDB.cardIDEnum），值为该卡牌被丢弃的次数。
    /// </summary>
    ...
    /// <summary>
    /// 存储己方被烧毁的卡牌及其数量。
    /// 键为卡牌ID（CardDB.cardIDEnum），值为该卡牌被烧毁的次数。
    /// 目前推测不会使用，暂时注释掉。
    /// </summary>
    public Dictionary<CardDB.cardIDEnum, int> ownBurnt = new Dictionary<CardDB.cardIDEnum, int>();
    ...
    /// <summary>
    /// 存储所有墓地相关的物品列表。
    /// 每个元素为一个GraveYardItem对象，表示墓地中的一项记录。
    /// </summary>
    List<GraveYardItem> graveyard = new List<GraveYardItem>();

    /// <summary>
    /// 存储当前回合中死亡的随从（MOBS）列表。
    /// 每个元素为一个GraveYardItem对象，表示本回合死亡的随从。
    /// </summary>
    public List<GraveYardItem> turngraveyard = new List<GraveYardItem>();

    /// <summary>
    /// 存储当前回合中所有死亡的单位（包括随从和其他类型）列表。
    /// 每个元素为一个GraveYardItem对象，表示本回合死亡的所有单位。
    /// </summary>
    public List<GraveYardItem> turngraveyardAll = new List<GraveYardItem>();

    /// <summary>
    /// 存储从上一回合开始到当前回合为止的所有墓地相关物品列表。
    /// 每个元素为一个GraveYardItem对象，表示这段时间内进入墓地的单位。
    /// </summary>
    List<GraveYardItem> graveyartTillTurnStart = new List<GraveYardItem>();

    /// <summary>
    /// 存储敌方当前部署的秘密法术列表。
    /// 每个元素为一个SecretItem对象，表示敌方的一个秘密法术。
    /// </summary>
    public List<SecretItem> enemySecrets = new List<SecretItem>();

        public bool 费尔根死亡 = false;
        public bool 斯塔拉格死亡 = false;
        public bool 倒抽 = false;
        ...
        public void setOwnCardsOut(Dictionary<CardDB.cardIDEnum, int> og)
        {
            ...
            {
            ownGraveyard.Add(tmp.Key, tmp.Value);

                // 检查是否为费尔根或斯塔拉格，且死亡前未被沉默
                if (tmp.Key == CardDB.cardIDEnum.FP1_015)
                {
                    // TODO: 需要检查该随从死亡时是否被沉默
                    // 如果未被沉默，则标记为死亡
                    this.费尔根死亡 = true;
                }
                if (tmp.Key == CardDB.cardIDEnum.FP1_014)
                {
                    // TODO: 需要检查该随从死亡时是否被沉默  
                    // 如果未被沉默，则标记为死亡
                    this.斯塔拉格死亡 = true;
        ...
        enemyGraveyard.Clear();
            ...
            {
            enemyGraveyard.Add(tmp.Key, tmp.Value);
        ...
        {
            string g = "";
            if (Probabilitymaker.Instance.费尔根死亡) g += " fgn";
            if (Probabilitymaker.Instance.斯塔拉格死亡) g += " stlgg";
    ...
    /// <summary>
    /// 打印玩家和敌方的墓地信息到日志中。
    /// 该函数遍历玩家和敌方的墓地字典，将每个卡牌的ID及其数量格式化为字符串，
    /// 并通过日志工具输出。主要用于调试或记录游戏状态。
    /// </summary>
