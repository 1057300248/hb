...
namespace HREngine.Bots
{
    ...
    public class Playfield
    {
        ...
        public int turn = 1; // ← 添加回合计数器
        public bool 法术费用增加 = false; //标志洛欧塞布效果是否生效
        public int 洛欧塞布法力值增加量 = 0; //洛欧塞布效果的法力值增加量
        ...
        public List<CardDB.Card> enemyDeck = new List<CardDB.Card>();//新增enemyDeck字段，用于记录敌方的牌库
        ...
        public List<CardDB.cardIDEnum> ownAuraIDList = new List<CardDB.cardIDEnum>();
        ...
        public int ownMinionsDied = 0;  //友方随从死亡数
        public int enemyMinionsDied = 0;  // 敌方随从死亡数
        public bool anzSolor = false;//日蚀
        public int ownMinionStartCount = 0;//回合开始时己方随从数
        public int enemyMinionStartCount = 0;//回合开始时敌方随从数
        public bool 孵化池突袭 = false;
        public int spellsplayedThisTurn = 0;//本回合施放的法术数量
        public int healOrDamageTimes = 0;//本回合治疗或受伤次数，巨人用

        public int 一费卡牌使用次数 = 0;//一费卡牌使用次数，口渴的流浪者用
        ...
        public triggerCounter tempTrigger = new triggerCounter();//触发计数器记录计数
        ...
        public int 水蛭伤害 = 0;//丑恶的残躯
        public int 敌水蛭伤害 = 0;//丑恶的残躯
        ...
        public int 复制抽到的牌 = 0;//克洛玛古斯
        public int 敌方复制抽到的牌 = 0;
        public int 龙族减费 = 0;//龙王配偶
        public int anzOwnMurlocConsort = 0;
        public int 龙族减费牌刚使用 = 0;
        ...
        public int 全息技师 = 0;//全息技师
        public bool 倒抽 = false;//牌库倒抽
        ...
        public int 下回合对方抽牌一次就扣两血计数器 = -1; // -1表示未激活  灰烬元素专用
        //#########################################
        ...
        public bool 费尔根死亡 = false;//费尔根死亡标记
        public bool 斯塔拉格死亡 = false;//斯塔拉格死亡标记
        public bool weHavePlayedMillhouseManastorm = false;//米尔豪斯·法力风暴
        public bool 我们有热砂港狙击手 = false;//热砂港狙击手
        public bool 敌方有热砂港狙击手 = false;
        ...
        public CardDB.cardIDEnum ownLastDraeneiCardId = CardDB.cardIDEnum.None;
        public CardDB.cardIDEnum enemyLastDraeneiCardId = CardDB.cardIDEnum.None;
        public int enemydoublepriest = 0;
        public int ownMistcaller = 0;//唤雾者伊戈瓦尔
        public int lockandload = 0;//子弹上膛
        public int stampede = 0;//奔踏
        public int 我们有瑞文戴尔男爵 = 0;//瑞文戴尔男爵
        public int 敌方有瑞文戴尔男爵 = 0;
        public int 友方技能额外触发次数 = 0;//友方技能额外触发次数
        ...
        public int 本局获得尸体数 = 0;
        public int 本局消耗尸体数 = 0;
        ...
        public bool 我方背刺王 = false;
        public bool 敌方背刺王 = false;
        //本回合对敌方英雄造成的伤害
        ...
        public int tarslick = 0; // 焦油飞溅
        public bool ownSunsapperlynessa = false;// 阳光汲取者莱妮莎
        public List<Handmanager.Handcard> watercolorArtistHand = new List<Handmanager.Handcard>();
        //海潮之王泰德 法术消耗变为5
        public bool spellsCost5UntilNextTurn = false;
        //海潮之王泰德 光环持续时间
        public int effectEndTurn = 0;
        //异教低阶牧师
        public int enemySpellCostOffsetNextTurn = 0;
        //海滩导购
        public Dictionary<int, Minion> nextNonRogueClassCardCostReduction = new Dictionary<int, Minion>();
        //冲浪章鱼
        public bool ownNextSpellCastWhenDrawn = false;
        //标记本回合英雄可以对相邻随从造成伤害
        public bool ownHeroHasCleave = false;
        //标记本回合敌方英雄可以对相邻随从造成伤害
        public bool enemyHeroHasCleave = false;
        //面板位置上限
        public int ownMinionsMaxSlots = 7;
        public int enemyMinionsMaxSlots = 7;
        //孤胆游侠雷诺 光环持续时间
        public int renoEffectEndTurn = 0;
        //伞降咒符
        public int sigilOfSkydivingTrigger = 0;
        //征战平原
        //public List<int> ownFieldOfStrifeDurations = new List<int>();
        public List<int> enemyFieldOfStrifeDurations = new List<int>();
        ...
        //卡牌使用逻辑    一费卡牌使用次数计数
        public void PlayCard(Handmanager.Handcard hc, Minion target, int choice)
        {
        ...
        private IEnumerable<Minion> GetAllMinions()
        {
            return ownMinions.Concat(enemyMinions);
        }
        ...
        public int getCorpseCount()
        {
            ...
            return retCount;
           // return Math.Max(0, this.本局获得尸体数 - this.本局消耗尸体数);
        }
        ...
        public Playfield()
        {
            ...
            this.一费卡牌使用次数 = 0;//一费卡牌使用次数，口渴的流浪者用
            this.healTimes = 0;
            this.totalAngr = -1;
            ...
            this.费尔根死亡 = Probabilitymaker.Instance.费尔根死亡;//这里识别费尔根的死亡状态
            this.斯塔拉格死亡 = Probabilitymaker.Instance.斯塔拉格死亡;//这里识别斯塔拉格死亡状态
            this.倒抽 = Probabilitymaker.Instance.倒抽;
            ...
            foreach (Minion m in this.ownMinions)
            {
                ...
                switch (m.name)
                {
                    ...
                    case CardDB.cardNameEN.scargil: // 斯卡基尔
                        this.anzOwnScargil++;
                        continue;
                    case CardDB.cardNameEN.nagaseawitch://纳迦海巫
                        this.纳迦海巫++;
                        continue;
                    case CardDB.cardNameEN.aviana://艾维娜
                        this.anzOwnAviana++;
                        continue;
                    case CardDB.cardNameEN.razorscale://锋鳞
                        this.锋鳞++;
                        continue;
                    ...
                    case CardDB.cardNameEN.baronrivendare://瑞文戴尔男爵
                        this.我们有瑞文戴尔男爵++;
                        continue;
                    case CardDB.cardNameEN.singalongbuddy://伴唱机
                        this.友方技能额外触发次数++;
                        continue;
                    ...
                    case CardDB.cardNameEN.raidleader://团队领袖
                        this.anzOwnRaidleader++;
                        continue;
                    case CardDB.cardNameEN.hideoushusk://丑恶的残躯
                        this.水蛭伤害 += 2;
                        continue;
                    ...
                    case CardDB.cardNameEN.stormwindchampion://暴风城勇士
                        ...
                        continue;
                    case CardDB.cardNameEN.zilliaxdeluxe3000:
                        this.奇利亚斯加攻击++;
                        continue;
                    ...
                    case CardDB.cardNameEN.acidmaw://酸喉
                        this.anzAcidmaw++;
                        continue;
                    case CardDB.cardNameEN.holotechnician:
                        this.全息技师++;
                        continue;
        ...
        public Playfield(Playfield p, bool transToEnemy = false)
        {
            ...
            this.一费卡牌使用次数 = p.一费卡牌使用次数;//一费卡牌使用次数，口渴的流浪者用
            ...
            this.ownAuraIDList.AddRange(p.ownAuraIDList);
            ...
            if (transToEnemy)
            {
                ...
                this.anzTamsin = this.ownHero.enchs.IndexOf(CardDB.cardIDEnum.SW_091t5) > 0;
                ...
                if (this.ownHero.enchs.IndexOf(CardDB.cardIDEnum.SW_450t4e) > 0)
                {
                    this.spellpower += 3;
                }
            ...
            this.本局获得尸体数 = p.本局获得尸体数;
            this.本局消耗尸体数 = p.本局消耗尸体数;
            //圣契
            ...
            this.复制抽到的牌 = p.复制抽到的牌;
            this.敌方复制抽到的牌 = p.敌方复制抽到的牌;
            this.龙族减费 = p.龙族减费;
            this.anzOwnMurlocConsort = p.anzOwnMurlocConsort;   //酸小明
            this.龙族减费牌刚使用 = p.龙族减费牌刚使用;
            //火光元素
            ...
            this.我们有瑞文戴尔男爵 = p.我们有瑞文戴尔男爵;
            this.敌方有瑞文戴尔男爵 = p.敌方有瑞文戴尔男爵;
            ...
            this.我们有热砂港狙击手 = p.我们有热砂港狙击手;
            this.敌方有热砂港狙击手 = p.敌方有热砂港狙击手;
            //#########################################
            this.tempanzOwnCards = this.owncards.Count;
            this.tempanzEnemyCards = this.enemyAnzCards;
            this.value = int.MinValue;
            this.友方技能额外触发次数 = p.友方技能额外触发次数;
...
*/
            ...
            if (this.ownAuraIDList.Count != p.ownAuraIDList.Count)
            {
                if (logg) Helpfunctions.Instance.logg("aurasCount changed");
                return false;
            }
            for (int i = 0; i < this.ownAuraIDList.Count; i++)
            {
                if (this.ownAuraIDList[i] != p.ownAuraIDList[i])
                {
                    if (logg) Helpfunctions.Instance.logg("auras changed");
                    return false;
                }
            }
            return true;
        ...
        public void guessHeroDamage()
        {
            ...
            foreach (Minion m in this.enemyMinions)
            {
                ...
                switch (m.name)
                {
                    case CardDB.cardNameEN.imposinganubisath: if (!m.silenced) continue; break;//威严的阿努比斯不能攻击
        ...
        public void simulateTrapsEndEnemyTurn()
        {
            ...
            //更新海滩导购
            Hrtprozis.Instance.nextNonRogueClassCardCostReduction = this.nextNonRogueClassCardCostReduction;
            //更新冲浪章鱼
            Hrtprozis.Instance.ownNextSpellCastWhenDrawn = this.ownNextSpellCastWhenDrawn;
            //更新上次打出的卡牌的费用
            Hrtprozis.Instance.lastPlayedCardCost = this.lastPlayedCardCost;
            //移除横扫效果
            this.ownHeroHasCleave = false;
            this.enemyHeroHasCleave = false;
            Hrtprozis.Instance.spellsCost5UntilNextTurn = this.spellsCost5UntilNextTurn;
            Hrtprozis.Instance.effectEndTurn = this.effectEndTurn;
            Hrtprozis.Instance.enemySpellCostOffsetNextTurn = this.enemySpellCostOffsetNextTurn;
            Hrtprozis.Instance.ownMinionsMaxSlots = this.ownMinionsMaxSlots;
            Hrtprozis.Instance.enemyMinionsMaxSlots = this.enemyMinionsMaxSlots;
            Hrtprozis.Instance.renoEffectEndTurn = this.renoEffectEndTurn;
            //伞降咒符
            Hrtprozis.Instance.sigilOfSkydivingTrigger = this.sigilOfSkydivingTrigger;
            //征战平原
            //Hrtprozis.Instance.ownFieldOfStrifeDurations = this.ownFieldOfStrifeDurations;
            Hrtprozis.Instance.enemyFieldOfStrifeDurations = this.enemyFieldOfStrifeDurations;
            //帕奇斯
            Hrtprozis.Instance.playfieldPatchesInDeck = this.patchesInDeck;
        }
        ...
        public void callKid(CardDB.Card c, int zonepos, bool own, bool spawnKid = true, bool oneMoreIsAllowed = false)
        {
            ...
            //处理猎潮者耐普图隆
            if (m.nameCN == CardDB.cardNameCN.猎潮者耐普图隆)
            {
                CardDB.Card kid1 = CardDB.Instance.getCardDataFromID(CardDB.cardIDEnum.TID_712t);
                CardDB.Card kid2 = CardDB.Instance.getCardDataFromID(CardDB.cardIDEnum.TID_712t2);
                if (own)
                {
                    if (this.ownMinions.Count <= 5)
                    {
                        this.callKid(kid1, 0, own);
                        this.callKid(kid2, +2, own);
                    }
                    else if (this.ownMinions.Count == 6)
                    {
                        this.callKid(kid1, 0, own);
                    }
                }
                ...
                {
                    if (this.enemyMinions.Count <= 5)
                    {
                        this.callKid(kid1, 0, own);
                        this.callKid(kid2, +2, own);
                    }
                    else if (this.enemyMinions.Count == 6)
                    {
                        this.callKid(kid1, 0, own);
                    }
                }
            ...
            if (m.nameCN == CardDB.cardNameCN.暴食巨鳗格拉格)
            {
                CardDB.Card kid1 = CardDB.Instance.getCardDataFromID(CardDB.cardIDEnum.TSC_639t);//squire
                ...
                if (own)
                {
                    if (this.ownMinions.Count <= 4)
                    {
                        this.callKid(kid1, +2, own);
                        this.callKid(kid2, +3, own);
                        this.callKid(kid3, +4, own);
                    }
                    else if (this.ownMinions.Count == 5)
                    {
                        this.callKid(kid2, +2, own);
                        this.callKid(kid3, +3, own);
                    }
                    else if (this.ownMinions.Count == 6)
                    {
                        this.callKid(kid3, +2, own);
                    }
                }
                else
                {
                    ...
                    else if (this.enemyMinions.Count == 5)
                    {
                        this.callKid(kid2, +2, own);
                        this.callKid(kid3, +3, own);
                    }
        ...
        public void endTurn()
        {
            if (turnCounter == 下回合对方抽牌一次就扣两血计数器)
            {
                下回合对方抽牌一次就扣两血计数器 = -1; // 清除标记
            }
            turnCounter++; // 递增回合计数器
        ...
        public void startTurn()
        {
            ...
            // 检查是否达到法术费用效果的结束时间，并确保effectEndTurn被正确设置
            if (this.effectEndTurn > 0 && this.turnCounter >= this.effectEndTurn)
            {
                this.spellsCost5UntilNextTurn = false; // 恢复正常法术消耗
                this.effectEndTurn = 0; // 重置effectEndTurn，避免多次触发
            }

            // 检查是否达到雷诺效果的结束时间，并确保renoEffectEndTurn被正确设置
            if (this.renoEffectEndTurn > 0 && this.turnCounter >= this.renoEffectEndTurn)
            {
                // 恢复正常的面板位置上限
                this.ownMinionsMaxSlots = 7;
                this.enemyMinionsMaxSlots = 7;
                this.renoEffectEndTurn = 0; // 重置renoEffectEndTurn，避免多次触发
            }

            //召唤坠落的伊利达雷
            if (isOwnTurn && sigilOfSkydivingTrigger > 0)
            {
                for (int i = 0; i < sigilOfSkydivingTrigger; i++)
                {
                    SummonIllidari();
                }
                sigilOfSkydivingTrigger = 0;
            }

        }

        /// <summary>
        /// 召唤坠落的伊利达雷的函数，检查随从数量限制
        /// </summary>
        /// <param name="p"></param>
        private void SummonIllidari()
        {
            CardDB.Card illidari = CardDB.Instance.getCardDataFromID(CardDB.cardIDEnum.VAC_926t); // 获取坠落的伊利达雷

            int maxMinions = 7; // 随从数量上限
            int availableSpaces = maxMinions - this.ownMinions.Count; // 计算当前场上剩余的空位

            if (availableSpaces > 0) // 确保场上有空位
            {
                int summonCount = Math.Min(3, availableSpaces); // 最多召唤3个，不能超过空位数

                for (int i = 0; i < summonCount; i++)
                {
                    this.callKid(illidari, this.ownMinions.Count, true); // 召唤伊利达雷
                }
            }
        }
            ...
            switch (a.actionType)
            {
                ...
                case actionEnum.useStarship:
                    ...
                    if (a.target != null)
                    {
                        Minion targetMinon = FindMinionByEntityId(new Minion { entitiyID = a.target.entitiyID });
                        // 使用星舰
                        useStarship(a.own, targetMinon);
                    }
                    else
                    {
                        // 使用星舰
                        useStarship(a.own, a.target);
                    }
                    // 计算英雄技能执行后敌方英雄的生命值差
        ...
        private void HandleMinionAttack(Action a)
        {
            ...
            {
                int tmpangr = 0;
                if (a.own.nameCN == CardDB.cardNameCN.耐普图隆之手)
                {
                    a.own.immune = true;
                }

                if (a.own.nameCN == CardDB.cardNameCN.猎潮者耐普图隆)
                {
                    if (!a.own.silenced)
                    {
                        int mydmg = a.own.Angr;
                        tmpangr = mydmg;
                        int kidnum = 0;
                        foreach (var item in this.ownMinions)
                        {
                            if (item.nameCN == CardDB.cardNameCN.耐普图隆之手)
                            {
                                kidnum += item.Angr;
                            }
                        }
        ...
        public void triggerAMinionAttacks(Minion attackingMinion)
        {
            // 判断随从是否为己方随从
            if (attackingMinion.own)
            {
                // 遍历己方随从并触发对应的召唤事件
                foreach (Minion mnn in this.ownMinions.ToArray())
                {
                    // 排除自身、沉默的随从
                    if (mnn.silenced || mnn == attackingMinion) continue; // 添加自身判断
                    mnn.handcard.card.sim_card.onMinionAttacks(this, mnn, attackingMinion);
                }
            }
            else
            {
        ...
        public void minionAttacksMinion(Minion attacker, Minion defender, bool dontcount = false)
        {
            ...
            // +++ 新增触发点：在攻击开始时触发其他随从的效果 +++
            this.triggerAMinionAttacks(attacker);
            // 处理英雄攻击的特殊情况
            if (attacker.isHero)
            {
                ...
                // 添加反击伤害逻辑
                int counterDmg = defender.Angr;
                attacker.getDamageOrHeal(counterDmg, this, true, false);
                return;
            }
        ...
        private void HandlePostAttackEffects(Minion attacker, Minion defender, bool dontcount)
        {
            ...
            {
                case CardDB.cardNameEN.allterrainvoidhound:
                    this.minionGetTempBuff(this.ownHero, 5, 0);
                ...
                case CardDB.cardNameEN.giantsandworm:
                    ...
                    break;
                case CardDB.cardNameEN.mutalisk:
                ...
                case CardDB.cardNameEN.gnomeliasafepilot:
        ...
        public void attackWithWeapon(Minion hero, Minion target, int penality)
        {
            ...
            switch (weapon.name)
            {
                ...
                case CardDB.cardNameEN.foolsbane:
                    ...
                    break;
                case CardDB.cardNameEN.auctionhousegavel:
                    if (own)
                    {
                        // 为战吼牌减费
                        Handmanager.Handcard hc = this.searchRandomMinionInHand(this.owncards, searchmode.searchLowestCost, GAME_TAGs.BATTLECRY);
                        if (hc != null)
                        {
                            hc.manacost--;
                        }
        ...
        /*// 在召唤随从的方法中
 public void SummonMinion(Minion summonedMinion, bool isOwn)
 {
     // ... 其他逻辑 ...

     // 分步检查空引用
     if (summonedMinion.handcard != null && 
         summonedMinion.handcard.card != null)
     {
         // 获取卡牌种族
         CardDB.Race race = summonedMinion.handcard.card.race;

         // 检查是否为德莱尼
         if (race == CardDB.Race.DRAENEI)
         {
             if (isOwn)
             {
                 playfield.ownLastDraeneiId = summonedMinion.handcard.card.cardIDenum;
             }
             else
             {
                 playfield.enemyLastDraeneiId = summonedMinion.handcard.card.cardIDenum;
             }
         }
 }*/
        ...
        private void HandleSpellCardEffects(Handmanager.Handcard hc, Minion target, int choice)
        {
            ...
            // 处理光环牌
            if (hc.card.Objective != 0)
            {
                this.ownAuraIDList.Add(hc.card.cardIDenum);
            }
        }
        ...
        private void HandleWeaponPlay(Handmanager.Handcard hc, Minion target, int choice)
        {
            ...
            {
                hc.card.sim_card.getBattlecryEffect(this, null, hc.target, choice);
        ...
        private void TriggerMinionDiedEffects(List<Minion> minions)
        {
            ...
            {
                if (m.Hp > 0) continue;
                foreach (Minion checkdm in minions.ToArray())
                {
                    if (checkdm.silenced || checkdm.Hp <= 0) continue;
                    checkdm.handcard.card.sim_card.onMinionDiedTrigger(this, checkdm, m); // 触发随从的死亡效果
                }
            }
        }
        ...
        public void triggerAMinionIsGoingToAttack(Minion attacker, Minion target)
        {
            ...
            switch (attacker.name)
            {
                ...
                case CardDB.cardNameEN.bloodboundimp: // "血缚小鬼"：如果攻击英雄，对我方英雄造成2点伤害
                    minionGetDamageOrHeal(attacker.own ? this.ownHero : this.enemyHero, 2);
                    break;
            ...
            // 十字军光环
            int buff = 0;
            foreach (CardDB.cardIDEnum s in this.ownAuraIDList)
            {
            ...
            this.minionGetBuffed(attacker, 2 * buff, 1 * buff);
        ...
        public void triggerACardWillBePlayed(Handmanager.Handcard hc, bool own)
        {
            ...
            {
                // 处理特殊随从效果
                if (龙族减费 > 0 && (TAG_RACE)hc.card.race == TAG_RACE.DRAGON) 龙族减费牌刚使用 = 0; // 龙族减费牌效果
                ...
                foreach (Minion m in this.enemyMinions)
                {
                    ...
                    if (m.handcard.card.nameCN == CardDB.cardNameCN.剑圣奥卡尼 && hc.card.type == CardDB.cardtype.MOB)
                    {
                        nb++;
                    }
                }
                ...
                for (int i = 0; i < nb; i++)
                {
                    this.callKid(CardDB.Instance.getCardDataFromID(CardDB.cardIDEnum.GDB_142), this.enemyMinions.Count, !own);
                    foreach (Minion m in this.enemyMinions)
                    {
                        if (m.handcard.card.cardIDenum == CardDB.cardIDEnum.GDB_142)
                        {
                            m.taunt = true;
                            if (m.own) this.anzOwnTaunt++;
                            else this.anzEnemyTaunt++;
        ...
        public void triggerAMinionWasSummoned(Minion mnn)
        {
            if (mnn.own)
            {
                ...
                // 根据fieldOfStrifeDurations.Count增加己方召唤随从的攻击力
                //if (this.ownFieldOfStrifeDurations.Count > 0)
                //{
                // 征战平原效果叠加，每个效果增加+1攻击力
                //  int attackBuff = this.ownFieldOfStrifeDurations.Count;
                // this.minionGetBuffed(mnn, attackBuff, 0); // 增加攻击力，不增加生命值
                // }
        ...
        public void triggerStartTurn(bool ownturn)
        {
            ...
            foreach (Handmanager.Handcard hc in this.owncards)
            {
                hc.UpdateQuickdrawStatus(this.prozis.gTurn);
            }
        ...
        public void doDeathrattles(List<Minion> deathrattleMinions)
        {
            ...
            foreach (Minion m in deathrattleMinions)
            {
                ...
                {
                    // 根据阵营获取对应随从列表（直接引用列表避免复制）
                    List<Minion> candidates = m.own
                        ? this.ownMinions
                        : this.enemyMinions;

                    // 使用更简洁的随机选择方式（模仿参考方法的逻辑）
                    Minion target = (m.own)
                        ? this.searchRandomMinion(this.ownMinions, searchmode.searchLowestAttack)
                        : this.searchRandomMinion(this.enemyMinions, searchmode.searchLowestAttack);

                    if (target != null)
                    {
                        // 应用强化效果
                        this.minionGetBuffed(target, 2, 2);
                ...
                {
                    CardDB.Card kid = CardDB.Instance.getCardDataFromID(CardDB.cardIDEnum.DMF_734); // 厚甲战马
                    int pos = m.own ? this.ownMinions.Count : this.enemyMinions.Count;
                    callKid(kid, pos, m.own, false, true);
                }
                ...
                // 处理里文戴尔男爵效果，双倍触发亡语
                if ((m.own && this.我们有瑞文戴尔男爵 >= 1) || (!m.own && this.敌方有瑞文戴尔男爵 >= 1))
                {
                    int r = m.own ? this.我们有瑞文戴尔男爵 : this.敌方有瑞文戴尔男爵;
                    for (int j = 0; j < r; j++)
                    {
                        if (!m.silenced && m.handcard.card.deathrattle)
                        {
        ...
        public void minionGetOrEraseAllAreaBuffs(Minion m, bool get)
        {
            ...
            if (m.own)
            {
                ...
                // 处理己方随从的增益效果
                angr += anzOwnRaidleader + anzOwnStormwindChamps + 奇利亚斯加攻击 + (this.ueberladung > 0 || this.lockedMana > 0 ? anzOwnVessina : 0);
        ...
        /// <summary>
        /// 抽取一张指定种族的随从卡牌并添加到玩家手牌中。
        /// </summary>
        /// <param name="race">要抽取的随从种族。</param>
        /// <param name="own">如果为 true，则表示为己方玩家抽卡；否则为敌方玩家抽卡。</param>
        /// <param name="nopen">如果为 true，则不进行抽卡操作。</param>
        public void drawACard(CardDB.Race race, bool own, bool nopen = false)
        {
            // 检查是否为己方玩家抽卡
            if (own)
            {
                // 处理未知种族（即从牌库顶抽取的卡牌）
                if (race == CardDB.Race.INVALID && !nopen)
                {
                    // 牌库为空，触发疲劳伤害
                    if (ownDeckSize == 0)
                    {
                ...
                if (复制抽到的牌 > 0 && race == CardDB.Race.INVALID && !nopen)
                {
                    for (int i = 1; i <= 复制抽到的牌; i++)
                    {
                        if (this.owncards.Count >= 10)
                        {
                            return;
            ...
            else // 处理敌方玩家抽卡
            {
                ...
                if (敌方复制抽到的牌 > 0 && race == CardDB.Race.INVALID && !nopen)
                {
                    for (int i = 1; i <= 敌方复制抽到的牌; i++)
                    {
                        if (this.enemyAnzCards >= 10)
                        {
                            return;
            ...
            // 处理未知种族的抽取和添加到手牌中
            if (race == CardDB.Race.INVALID)
            {
                CardDB.Card c = CardDB.Instance.getCardData(CardDB.cardNameEN.unknown);
            ...
            {
                // 从卡牌数据库中随机获取一张指定种族的随从卡牌
                List<CardDB.Card> availableCards = new List<CardDB.Card>();
                foreach (var cardName in Enum.GetValues(typeof(CardDB.cardNameEN)))
                {
                    CardDB.Card card = CardDB.Instance.getCardData((CardDB.cardNameEN)cardName);
                    if (card != null && card.race == race && card.type == CardDB.cardtype.MOB)
                    {
                ...
                if (availableCards.Count > 0)
                {
                    ...
                    CardDB.Card c = availableCards[rand.Next(availableCards.Count)];
                    Handmanager.Handcard hc = new Handmanager.Handcard
                    {
                        card = c,
                        position = this.owncards.Count + 1,
        ...
        public void drawACard(CardDB.cardNameEN ss, bool own, bool nopen = false)
        {
            ...
            if (!own && turnCounter == 下回合对方抽牌一次就扣两血计数器)
            {
                // 对敌方英雄造成2点伤害
                minionGetDamageOrHeal(enemyHero, 2);
            }
            ...
            if (own)
            {
                ...
                // 处理己方的Chromaggus（克洛玛古斯）效果，额外抽卡
                if (复制抽到的牌 > 0 && s == CardDB.cardNameEN.unknown && !nopen)
                {
                    for (int i = 1; i <= 复制抽到的牌; i++)
                    {
                        if (this.owncards.Count >= 10)
            ...
            else // 处理敌方玩家抽卡
            {
                ...
                // 处理敌方的Chromaggus（克洛玛古斯）效果，额外抽卡
                if (敌方复制抽到的牌 > 0 && s == CardDB.cardNameEN.unknown && !nopen)
                {
                    for (int i = 1; i <= 敌方复制抽到的牌; i++)
                    {
                        if (this.enemyAnzCards >= 10)
        ...
        public void drawACard(CardDB.cardIDEnum ss, bool own, bool nopen = false)
        {
            ...
            if (own)
            {
                ...
                // 处理己方的Chromaggus（克洛玛古斯）效果，额外抽卡
                if (复制抽到的牌 > 0 && s == CardDB.cardIDEnum.None && !nopen)
                {
                    for (int i = 1; i <= 复制抽到的牌; i++)
                    {
                        if (this.owncards.Count >= 10)
            ...
            else // 处理敌方玩家的抽卡逻辑
            {
                ...
                // 处理敌方的Chromaggus（克洛玛古斯）效果，额外抽卡
                if (敌方复制抽到的牌 > 0 && s == CardDB.cardIDEnum.None && !nopen)
                {
                    for (int i = 1; i <= 敌方复制抽到的牌; i++)
                    {
                        if (this.enemyAnzCards >= 10) // 手牌已满（10张），无法再抽牌
        ...
        /// <summary>
        /// 选择生命值最低的一个敌方角色作为单次伤害的目标，优先选择随从但会根据实际情况调整。
        /// </summary>
        /// <param name="damage">伤害值（此参数在逻辑中未直接使用）。</param>
        /// <param name="onlyMinions">是否只选择随从作为目标。</param>
        /// <returns>选择的目标角色。</returns>
        public Minion getEnemyCharTargetForRandomSingleDamage1(int damage, bool onlyMinions = false)
        {
            Minion target = null;
            int tmp = this.guessingHeroHP; // 保存当前猜测的英雄血量
            this.guessHeroDamage();        // 计算可能影响英雄血量的临时值

            // 获取敌方随从中生命值最低的随从（可能有多个相同值，随机选一个）
            Minion lowestHPMinion = this.searchRandomMinion(this.enemyMinions, searchmode.searchLowestHP);

            if (!onlyMinions)
            {
                // 计算敌方英雄和最低HP随从的生命值
                int heroHP = this.enemyHero.Hp;
                int minionHP = lowestHPMinion != null ? lowestHPMinion.Hp : int.MaxValue;

                // 如果敌方英雄生命值更低，则选英雄
                if (heroHP < minionHP)
                {
                    target = this.enemyHero;
                }
                else
                {
                    target = lowestHPMinion;
                    this.evaluatePenality -= target.Angr * 7;
                }
            }
            else
            {
            ...
            this.guessingHeroHP = tmp; // 恢复原始猜测血量
            return target;
        }
        ...
        public void discardCards(int num, bool own)
        {
            ...
            {
                // 记录异常但不中断游戏流程
                Helpfunctions.Instance.ErrorLog(string.Format("弃牌时发生错误: {0}", ex.Message));
            }
        }
        ...
        /// <summary>
        /// 处理剩余需要弃掉的卡牌，确保不会发生数组越界异常。
        /// </summary>
        /// <param name="anz">剩余需要弃掉的卡牌数量。</param>
        /// <param name="numDiscardedCards">实际弃掉的卡牌数量。</param>
        /// <param name="cardsValDict">卡牌价值字典，键为卡牌实体ID，值为卡牌价值。</param>
        /// <param name="discardedCardsBonusList">弃牌过程中获得奖励的卡牌列表。</param>
        ...
        {
            // 添加安全检查，防止数组越界
            if (anz <= 0 || this.owncards == null || this.owncards.Count == 0)
            {
                return;
            }

            // 确保弃牌数量不超过实际手牌数量
            int actualDiscardCount = Math.Min(anz, this.owncards.Count);

            for (int i = 0; i < actualDiscardCount; i++)
            {
                // 安全获取要弃掉的卡牌
                Handmanager.Handcard removedhc = this.owncards[0]; // 始终从索引0开始移除

                // 检查卡牌实体ID是否存在于价值字典中
                int bestCardValue = 0;
                if (cardsValDict.ContainsKey(removedhc.entity))
                {
                    bestCardValue = cardsValDict[removedhc.entity];
                }
        ...
        public int calDirectDmg(int mana, bool force, bool calNextTurn = false, int maxCal = 15, int calMax = 15)
        {
            ...
            foreach (Handmanager.Handcard hc in this.owncards)
            {
                ...
                // 走A模式
                if (this.ownHero.enchs.IndexOf(CardDB.cardIDEnum.SCH_617e) > 0)
                {
                    dmg[cnt] += 2;
                }
        ...
        private void getHandcardsByType(List<Handmanager.Handcard> cards, GAME_TAGs tag, TAG_RACE race = TAG_RACE.INVALID)
        {
            switch (tag)
            {
                ...
                case GAME_TAGs.BATTLECRY:
                    // 筛选具有战吼属性的卡牌
                    foreach (Handmanager.Handcard hc in cards)
                    {
                        if (hc.card.battlecry)
                        {
                            hc.extraParam3 = true;
                        }
                    }
            ...
            if (this.totalAngr == -1)
            {
                ...
                for (int i = 0; i < this.ownMinions.Count; i++)
                {
                    ...
                    if (this.ownMinions[i].Ready && !this.ownMinions[i].cantAttackHeroes)
                    {
                        ...
                        if (this.ownMinions[i].superwindfury)
                        {
                            if (this.ownMinions[i].numAttacksThisTurn == 0)
                            {
                                this.totalAngr += this.ownMinions[i].Angr;
                            }
                            if (this.ownMinions[i].numAttacksThisTurn == 1)
                            {
                                this.totalAngr += this.ownMinions[i].Angr;
                            }
                            if (this.ownMinions[i].numAttacksThisTurn == 2)
                            {
                                this.totalAngr += this.ownMinions[i].Angr;
                            }
                        }
        ...
        public void printBoard()
        {
            ...
            Helpfunctions.Instance.logg("我方尸体数: " + this.getCorpseCount());
            ...
            Helpfunctions.Instance.logg("敌方英雄血量: " + this.enemyHero.Hp + " + " + this.enemyHero.armor + ((this.enemyHero.immune) ? " immune" : "") + ((this.enemyHero.divineshild) ? " 圣盾" : ""));
        ...
        public string printBoardString()
        {
            ...
            retval += "\r\n" + ("Hp : armor : Atk ownhero\t" + this.ownHero.Hp + ":" + this.ownHero.armor + ":" + this.ownHero.Angr + ((this.ownHero.immune) ? ":immune" : "") + ((this.ownHero.divineshild) ? ":圣盾" : ""));
            ...
            retval += "\r\n" + ("Hp : armor enemyhero\t" + this.enemyHero.Hp + ":" + this.enemyHero.armor + ((this.enemyHero.immune) ? ":immune" : "") + ((this.enemyHero.divineshild) ? ":圣盾" : ""));
            ...
            retval += "\r\n" + ("ownCorpses\t" + this.getCorpseCount());
        ...
        /// 使用星舰卡牌
        /// </summary>
        /// <param name="own">星舰卡牌所属的随从对象</param>
        /// <param name="target">目标随从或英雄</param>
        public void useStarship(Minion own, Minion target)
        {
            // 获取当前要使用的星舰卡牌
            CardDB.Card starshipCard = own.handcard.card;
            bool flag = true;

            // 触发星舰的使用效果
            starshipCard.sim_card.useStarship(this, own, target);
        }
        ...
        public void drawTemporaryCard(CardDB.cardIDEnum ss, bool own)
        {
            ...
            else
            {
                ...
                if (敌方复制抽到的牌 > 0 && s == CardDB.cardIDEnum.None)
                {
                    for (int i = 1; i <= 敌方复制抽到的牌; i++)
                    {
                        if (this.enemyAnzCards >= 10)
